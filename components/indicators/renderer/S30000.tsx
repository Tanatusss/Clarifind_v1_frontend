// src/components/indicators/renderers/S30000.tsx
"use client";
...
export default function S30000({ resp }: { resp: IndicatorDetailResponse }) {
  const rows: Row[] = Array.isArray(resp?.details?.rows) ? resp.details.rows : [];

  // ✅ กรองเฉพาะ % ที่ "ตั้งแต่ 50% ขึ้นไป"
  ...
  // สัดส่วนที่เข้าเงื่อนไข (กันซ้ำ + เรียงมากไปน้อย)
  ...
  const title = resp?.indicator?.name_th ?? "ปัจจุบันผู้ถือหุ้นต่างชาติถือหุ้นตั้งแต่ 50% ขึ้นไป";

  return (
    <BaseDetailCard title={title} tone="cyan">
      {/* ผู้ถือหุ้นต่างชาติที่ถือหุ้น ≥ 50% */}
      {holders.length > 0 && (
        <DetailSection
          icon={<Users className="h-5 w-5 text-cyan-300" />}
          label="ผู้ถือหุ้นต่างชาติที่ถือหุ้นตั้งแต่ 50% ขึ้นไป"
        >
      ...
      {/* สัดส่วนของผู้ถือหุ้นต่างชาติ (≥ 50%) */}
      {percents.length > 0 && (
        <DetailSection
          icon={<Percent className="h-5 w-5 text-cyan-300" />}
          label="สัดส่วนของผู้ถือหุ้นต่างชาติ (≥ 50%)"
        >
          {percents.map((p, i) => (
